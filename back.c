...
void actualizar_tablero(char *tablero , int fils , int cols){
    ...
    //printf("dasd\n");
    //print_tablero(vecinos,fils,cols,1);
    free(vecinos);
}
 ...
 */
char contar_vecinos(char *tablero , int pos_fil, int pos_col , int fils , int cols){
    char cant_vecinos = 0;
    ...
    for(int i=pos_fil-1 ; i<=pos_fil+1 ; i++){
        for(int j=pos_col-1 ; j<=pos_col+1 ; j++){
            ...
            if((i>=0 && i<fils) && (j>=0 && j<cols) && (i!=pos_fil || j!=pos_col)){
                cant_vecinos+=(tablero[i*cols + j]? 1:0);
                //printf("tablero en [%d,%d] es %d\n",i,j,tablero[i*cols+j]);
            }
        }
    }
    //printf("los vecinos son:%d\n",cant_vecinos);
    return cant_vecinos;
}
